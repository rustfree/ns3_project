...
main ()
{
  ...
  LogComponentEnable ("UanMacBcpMaca", LOG_LEVEL_ALL);
  ...
  std::string m_asciitracefile = "UanPacketSocketMacabi_Nc4_ns.asc";
  /*物理层phy的配置*/
  ...
  uan.SetMac ("ns3::UanMacBcpMaca");//设置macAloha的参数
  /*创建节点*/
 ...
 //发送端:nc0到nc10
  ...
  for(int index = 0; index < m_numNodes; index++){
      ...
      sendAppsVec[index].Start (Seconds (startTime + index*double(meanInterval_A)/5));
