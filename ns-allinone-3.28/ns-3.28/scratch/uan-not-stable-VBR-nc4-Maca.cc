...
main ()
{
  ...
  uan.SetMac ("ns3::UanMacMaca");//设置macAloha的参数
  /*创建节点*/
 ...
 //发送端:nc0到nc10
  ...
  DataApplicationHelper DataAppsend_A ("ns3::PacketSocketFactory", Address ());
  DataAppsend_A.SetAttribute ("Mean", DoubleValue(meanInterval_A));
  DataAppsend_A.SetAttribute ("Bound", DoubleValue(meanInterval_A*2.5));
  DataAppsend_A.SetAttribute ("PacketSize", UintegerValue (packetSize));

  DataAppsend_A.SetAttribute ("Remote", AddressValue(sinkAddress));

 

    
  DataApplicationHelper DataAppsend_B ("ns3::PacketSocketFactory", Address ());
  DataAppsend_B.SetAttribute ("Mean", DoubleValue(meanInterval_B));
  DataAppsend_B.SetAttribute ("Bound", DoubleValue(meanInterval_B*2.5));
  DataAppsend_B.SetAttribute ("PacketSize", UintegerValue (packetSize));

  DataAppsend_B.SetAttribute ("Remote", AddressValue(sinkAddress));
 

  vector< ApplicationContainer> sendAppsVec;
  for(int index = 0; index < m_numNodes; index++){
      if(index < 2)
          sendAppsVec.push_back(DataAppsend_A.Install (nc.Get (index)));//安装到节点0到节点9
      else
          sendAppsVec.push_back(DataAppsend_B.Install (nc.Get (index)));//安装到节点0到节点9
