...
ArpL3Protocol::Lookup (Ptr<Packet> packet, const Ipv4Header & ipHeader, Ipv4Address destination,
                       ...
                       Ptr<ArpCache> cache,
                       Address *hardwareDestination)//hardwareDestination可改值，传递ARP条目的MAC地址
{
...
void ArpL3Protocol::AddArpEntry(Ptr<NetDevice> device,Address macAddress,Ipv4Address destination)
{
  NS_LOG_FUNCTION (this);
  Ptr<ArpCache> cache = FindCache (device);
  ArpCache::Entry *entry = cache->Lookup (destination);
