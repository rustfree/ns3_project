...
NS_LOG_COMPONENT_DEFINE ("RandomSendPacketSocketExampleNew");
...
double Sendtime = 2000;
double Recvtime = 2100;
double Simutime = 2110;
...
void InstallMobility(NodeContainer nc ,int m_numNodes)
{
  ...
  //double m_length = 7.5, m_width = 4, m_depth = 0.8,m_real_depth = 0.3;
  ...
  //可选坐标列表
  double x_list[4] = {1, 3, 2};
  double y_list[4] = {2, 2, 5};
    for (int i = 0; i < m_numNodes; i++)//
      {
        double x = x_list[i];
        double y = y_list[i];
        pos->Add (Vector (x, y, 0.3));//nc的位置
