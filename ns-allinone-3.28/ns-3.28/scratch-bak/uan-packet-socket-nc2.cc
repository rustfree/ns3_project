...
main ()
{
  ...
  std::string m_asciitracefile = "UanPacketSocketNc.asc";
  /*物理层phy的配置*/
 ...
 //发送端:nc0
  ...
  DataAppsend.SetAttribute ("Bound", DoubleValue(50));
  DataAppsend.SetAttribute ("PacketSize", UintegerValue (200));
...
"abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcde");
 ...
 //发送端:nc1
/*
  DataApplicationHelper DataAppsend2 ("ns3::PacketSocketFactory", Address (sinkAddress));

  DataAppsend2.SetAttribute ("Mean", DoubleValue(15));
  DataAppsend2.SetAttribute ("Bound", DoubleValue(50));
  DataAppsend2.SetAttribute ("PacketSize", UintegerValue (1000));
 
  ApplicationContainer sendApps2 = DataAppsend.Install (nc.Get (1));//安装到节点0
  DataAppsend2.SetFill(sendApps.Get(0),
"abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcde");
  sendApps2.Start (Seconds (2.1));
  sendApps2.Stop (Seconds (200.0));
 */
